...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.categoryCompetition.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.category-competitions.update", [$categoryCompetition->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $categoryCompetition->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('competition') ? 'is-invalid' : '' }}" name="competition_id" id="competition_id" required>
                    @foreach($competitions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('competition_id') ? old('competition_id') : $categoryCompetition->competition->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_of_players') ? 'is-invalid' : '' }}" type="number" name="number_of_players" id="number_of_players" value="{{ old('number_of_players', $categoryCompetition->number_of_players) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('half_duration') ? 'is-invalid' : '' }}" type="number" name="half_duration" id="half_duration" value="{{ old('half_duration', $categoryCompetition->half_duration) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_of_teams_in_group') ? 'is-invalid' : '' }}" type="number" name="number_of_teams_in_group" id="number_of_teams_in_group" value="{{ old('number_of_teams_in_group', $categoryCompetition->number_of_teams_in_group) }}" step="1">
