...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.matchEvent.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.match-events.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('event_type') ? 'is-invalid' : '' }}" type="number" name="event_type" id="event_type" value="{{ old('event_type', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('player') ? 'is-invalid' : '' }}" name="player_id" id="player_id">
                    @foreach($players as $id => $entry)
                        <option value="{{ $id }}" {{ old('player_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('minute') ? 'is-invalid' : '' }}" type="text" name="minute" id="minute" value="{{ old('minute', '') }}">
