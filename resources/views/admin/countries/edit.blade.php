        ...
        <form method="POST" action="{{ route("admin.countries.update", [$country->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_default') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_default" id="is_default" value="1" {{ $country->is_default || old('is_default', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_default">{{ trans('cruds.country.fields.is_default') }}</label>
                </div>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.country.fields.is_default_helper') }}</span>
            </div>
            <div class="form-group">
