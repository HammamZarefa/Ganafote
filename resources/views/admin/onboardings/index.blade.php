...
@section('content')
@can('onboarding_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.onboardings.create') }}">
                ...
                <tbody>
                    @foreach($onboardings as $key => $onboarding)
                        <tr data-entry-id="{{ $onboarding->id }}">
                            ...
                            <td>
                                @can('onboarding_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.onboardings.show', $onboarding->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('onboarding_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.onboardings.edit', $onboarding->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('onboarding_delete')
                                    <form action="{{ route('admin.onboardings.destroy', $onboarding->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('onboarding_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.onboardings.massDestroy') }}",
